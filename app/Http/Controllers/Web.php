...
class Web extends Controller
{
    ...
    public function store(Request $request){
        ...
        if(!empty(Auth::user())){
        	...
        	if(Auth::user()->id != "1" && !empty($bank)){
        	    ...
        	    if($type->id == "1"){
        	        return redirect('profile/'.Crypt::encrypt($type->id))->with('error','Please enter Bank Account(NGN) which you want to convert first.!');   
        	    }else if($type->id == "2"){
        	        return redirect('profile/'.Crypt::encrypt($type->id))->with('error','Please enter PM Account number which you want to convert first.!');
        	    }else if($type->id == "3"){
        	        return redirect('profile/'.Crypt::encrypt($type->id))->with('error','Please enter BTC address which you want to convert first.!');   
        	    }
        	}
        }else{
        ...
        if(!empty($data) && $request->amount != ""){
            ...
            if(!empty($displayRate)){
                ...
                if($request->to_currency == "NGN"){
                    if($displayRate >= $this->rubies->min2_amount){
                        $amount = $displayRate;
                    }else{
                        $amount = $this->rubies->min2_amount;
                        $stringAmount = "Enter Mininum amount or greater ".$this->rubies->min2_amount." ".$request->to_currency;
                    }
                }else if($request->to_currency == "PM"){
                    if($displayRate >= $this->pmConfs->min2_amount){
                        $amount = $displayRate;
                    }else{
                        $amount = $this->pmConfs->min2_amount;
                        $stringAmount = "Enter Mininum amount or greater ".$this->pmConfs->min2_amount." ".$request->to_currency;
                    }
                }else if($request->to_currency == "BTC"){
                    if($displayRate >= $this->btcConfs->min2_amount){
                        $amount = $displayRate;
                    }else{
                        $amount = $this->btcConfs->min2_amount;
                        $stringAmount = "Enter Mininum amount or greater ".$this->btcConfs->min2_amount." ".$request->to_currency;
                    }
                }
                if($request->to_currency == "NGN"){
                    return response()->json(
                        array(
                            'amount' => bcdiv(str_replace(",","",$this->convert_number($amount)),1,2),
                            'success' => '1',
                            'msg' => $stringAmount != '' ? '<strong style="color:red;">'.$stringAmount.'</strong>' :'<h4 style="color:green;">You will receive <strong>'.bcdiv(str_replace(",","",$this->convert_number($amount)),1,2).' '.$request->to_currency.'</strong></h4>',
                            'btnValue' => $stringAmount != '' ? true : false
                        )
                    );   
                }else if($request->to_currency == "PM"){
                    return response()->json(
                    ...
                    return response()->json(
                        array(
                            ...
                            'success' => '1',
                            'msg' => $stringAmount != '' ? '<strong style="color:red;">'.$stringAmount.'</strong>' :'<h4 style="color:green;">You will receive <strong>'.$this->convert_number($amount).' '.$request->to_currency.'</strong></h4>',
                            'btnValue' => $stringAmount != '' ? true : false
                        )
                    );
                }
            ...
            if(strpos($result[9],"No Records Found") != true){
                ...
                if(empty($data)){
                    ...
                    if(isset($transferAmount) && $transferAmount != ""){
                        ...
                        $rave->txn_status = "1";
                        $save = $rave->save();
                        /*echo $this->convert_number($transferAmount->amount * $result[13]);die;
                        echo $result[13];die;*/
                        if($save){
                            ...
                            $transaction->transferAmount = $this->convert_number($transferAmount->amount * $result[13]);
                            ...
                            if(session('to_currency') == "BTC" || $transaction->to_currency == "BTC"){
                                $toAddress = BankDetail::where(['user_id'=>Auth::user()->id,'type_id'=>'3'])->first();
                                // $fromAddress = $this->btcConfs->btc_address;
                                $res = $this->getIndex();
                                if(!empty($res)){
                                    $fromAddress = $res->index;
                                }else{
                                    $fromAddress = 0;
                                }
    ...
    public function getIndex(){
        $client = new \GuzzleHttp\Client();
        $api = config('rave.ngrok').$this->btcConfs->wallet_address.'/accounts/'.$this->btcConfs->pub_key.'?password='.$this->btcConfs->bank_password.'';
    ...
    /*public function btcAutoTransfer($transactionID,$amount,$from,$to){
        ...
        $api = 'http://97a9be172116.ngrok.io/merchant/6108261b-94a2-4603-b035-a61932383540/payment?password='.$this->btcConfs->bank_password.'&to='.$to.'&amount='.$satoshi_amount.'&from='.$from.'';
        $response = $client->request('GET',$api);
        ...
        if(!empty($result) && $result != ""){
            $rave = new Raves;
            $rave->user_id = $userid;
            $rave->txn_id =  $result->txid;
            $rave->txn_Ref =  $result->tx_hash;
            $rave->txn_flwRef =  "";
    ...
    public function sendBtcTransfer($transactionID,$amount,$from,$to){
        if($amount <= $this->pmConfs->min_amount ){
            ...
            $amount = $this->convert_number($amount);
            try{
                $api = config('rave.ngrok').$this->btcConfs->wallet_address.'/payment?password='.$this->btcConfs->bank_password.'&to='.$to.'&amount='.$satoshi_amount.'&from='.$from.'';
                $response = $client->request('GET',$api);
                $statusCode = $response->getStatusCode();
                $result = json_decode($response->getBody()->getContents());   
            ...
            if(!empty($result) && $result != "" && !isset($result->error)){
                ...
                if($save){
                    ...
                    $transfer->type_id = "3";
                    $transfer->transaction_id = $transactionID;
                    $transfer->draccount = $this->btcConfs->btc_address;
                    $transfer->craccount = $to;
                    $transfer->ref_no = $rave->txn_id;
                    $transfer->amount = $amount;
                    $tsave = $transfer->save();
    ...
    public function sendAdminBtcTransfer($transactionID,$amount,$from,$to){
        $client = new \GuzzleHttp\Client();
        $satoshi_amount = $amount * 100000000;
        $amount = $this->convert_number($amount);
        try{
    ...
    public function ngnAutoTransfer($id,$amount){
        $data = Transaction::where(['id'=>$id,'from_currency'=>'PM','to_currency'=>'NGN','status'=>'0'])->with('user')->first();
        if(!empty($data)){
    ...
    public function autoTransferAdmin(Request $request){
        if($request->to_currency == "NGN"){
            ...
            $data = Transaction::where(['id'=>$request->id,'to_currency'=>$request->to_currency,'status'=>'0'])->with('user')->first();
            if(!empty($data)){
                $toAddress = BankDetail::where(['user_id'=>$data->user->id,'type_id'=>'3'])->first();
                $ind = $this->getIndex();
                if(!empty($ind)){
                    $fromAddress = $ind->index;
                }else{
                    $fromAddress = 0;
                }
                $res = $this->sendAdminBtcTransfer($request->id,$data->transferAmount,$fromAddress,$toAddress->account_no);
                // print_r(isset($res->error) != "1" && $res->error != "");die;
                // print_r(!$res->error);die;
                if($res->error != 1 || $res->error != "1" || $res->error ==''){
                    return response()->json(
                        array(
                            'success'=> '1',
